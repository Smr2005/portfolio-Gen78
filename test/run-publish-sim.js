...
(async () => {
  try {
    ...
    const rendererPath = path.resolve(projectRoot, 'server', 'renderers', 'template3Renderer-ssr.js');
    const { renderTemplate3SSR } = require(rendererPath);
    ...
    try {
      ...
      clientBundleUrl = manifest.files && (manifest.files['main.js'] || manifest.files['main.js.map'] || manifest.files['static/js/main.js']) ? (manifest.files['main.js'] || manifest.files['static/js/main.js']) : null;
      if (clientBundleUrl && !clientBundleUrl.startsWith('/')) {
        // canonicalize to root-relative
        clientBundleUrl = clientBundleUrl.replace(/^\//, '');
        clientBundleUrl = '/' + clientBundleUrl;
      }
    } catch (e) {
      // manifest not found — we'll still render but hydrate will be false
      clientBundleUrl = null;
    }

    const html = renderTemplate3SSR(data, { hydrate: !!clientBundleUrl, clientBundleUrl: clientBundleUrl });
    ...
    const server = app.listen(port, async () => {
      ...
      const snapshotPath = path.resolve(snapshotDir, 'template3-published.html');
      fs.writeFileSync(snapshotPath, body, 'utf8');

      console.log('Snapshot saved to:', snapshotPath);

      // Print small excerpt and server bundle reference
    ...
    console.error('Error running publish sim:', err);
    process.exit(2);
  }
})();
