...
(async () => {
  try {
    ...
    const ssr = require(path.resolve(projectRoot, 'server', 'renderers', 'template4Renderer-ssr.js'));
    const data = { name: 'Smoke Test User', title: 'Tester' };

    const ssrHtml = ssr.renderTemplate4SSR(data, { hydrate: false });
    ...
    const Template4 = clientRequire(path.resolve(projectRoot, 'client', 'src', 'templates', 'Template4.js')).default;
