...
(async () => {
  try {
    ...
    const ssr = require(path.resolve(projectRoot, 'server', 'renderers', 'template6Renderer-ssr.js'));
    const data = { name: 'Smoke Test User', title: 'Tester' };

    const ssrHtml = ssr.renderTemplate6SSR(data, { meta: { title: 'test' } }, false);
    ...
    const Template6 = clientRequire(path.resolve(projectRoot, 'client', 'src', 'templates', 'Template6.js'));
    const Template6Comp = Template6 && Template6.default ? Template6.default : Template6;

    const element = React.createElement(Template6Comp, { isPreview: false, userData: data });
